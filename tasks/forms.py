...
class TaskForm(StyledFormMixin, forms.ModelForm):
    ...
    def __init__(self, *args, viewer=None, **kwargs):
        ...
        # Un responsable (non RH/CEO/admin) ne peut affecter qu'à SON équipe.
        if viewer is not None and viewer.is_manager and not viewer.is_rh:
            from employees.models import Employee
            team_ids = list(
                Employee.objects.filter(manager__user=viewer).values_list("user_id", flat=True)
            ) + [viewer.id]
            qs = qs.filter(id__in=team_ids)
